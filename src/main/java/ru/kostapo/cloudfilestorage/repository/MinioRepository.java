...
import ru.kostapo.cloudfilestorage.exception.ObjectNotFoundException;
    ...
    public boolean isBucketExists(String bucketName) {
        try {
            ...
            return minioClient.bucketExists(BucketExistsArgs.builder().bucket(bucketName).build());
                 ...
                 NoSuchAlgorithmException | IOException | ServerException | XmlParserException |
                 InvalidKeyException e) {
            throw new StorageException("Can't check root bucket exists");
        }
    }

    public boolean isObjectExists(String objectPath) {
        try {
            log.info("checking object [{}] exists...", objectPath);
            ...
            if ("NoSuchKey".equals(e.errorResponse().code())) {
                throw new ObjectNotFoundException(String.format("Object [%s] not found", objectPath));
            } else {
                throw new StorageException("Error occurred while checking object existence");
            }
        } catch (InsufficientDataException | InternalException | InvalidResponseException |
                 NoSuchAlgorithmException | IOException | ServerException | XmlParserException |
                 InvalidKeyException e) {
            throw new StorageException("Storage service can't check object exists");
        }
    }
        ...
        if(isObjectExists(username + "/" + filePath)) {
            try {
                return minioClient.getObject(
                     ...
                     NoSuchAlgorithmException | IOException | ServerException | XmlParserException |
                     InvalidKeyException e) {
                throw new StorageException(String.format("Storage service can't download file [%s]", filePath));
            }
        }
        return null;
    }
